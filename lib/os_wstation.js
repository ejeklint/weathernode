...
function OS_WStation(dataLogger) {
    ...
    this.hid = new HID.HID(devices[0].path);

    // Initialization sequence for HID based weather stations from Oregon Scientific
    this.hid.write([0x20, 0x00, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00]);

    // Bind callback function to HID library
    this.hid.read(this.handleHIDReport.bind(this));
...
OS_WStation.prototype.decodeRainReport = function(data) {
    ...
    var totalRain = this.roundedDoubleFromBytes(data[9], data[8], 0.254);
    var rainRate = this.roundedDoubleFromBytes(data[3] & 0x0f, data[2], 0.254);
    var rain1hour = this.roundedDoubleFromBytes(data[5], data[4], 0.254);
    var rain24hour = this.roundedDoubleFromBytes(data[7], data[6], 0.254);

    var rainTotalSince = new Date(data[14] + 2000, data[13], data[12], data[11], data[10], 0, 0);

    var obj = {};
    obj.reading = 'rain';
    obj.batteryLevel = this.batteryLevel(data[0]);
...
OS_WStation.prototype.decodeTempAndHumidityReport = function(data) {
    ...
    var temp = this.roundedDoubleFromBytes(data[4] & 0x0f, data[3], 0.1);
    ...
    if (sensor === 1) {
        var dew = this.roundedDoubleFromBytes(data[7] & 0x0f, data[6], 0.1);
...
OS_WStation.prototype.decodeWindReport = function(data) {
    var self = this;
    var gust = this.roundedDoubleFromBytes(data[5] & 0x0f, data[4], 0.1);
    ...
    if (data[8] != 0x20) {
        var chill = (this.roundedDoubleFromBytes(data[8] & 0x0f, data[7], 0.1) - 32) / 1.8;
...
OS_WStation.prototype.decodeStatusReport = function(data) {
    ...
    var obj = {};
    obj.reading = 'status';
    obj.battery_level = this.batteryLevel(data[0]);
