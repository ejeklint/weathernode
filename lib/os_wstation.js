...
function OS_WStation() {
    ...
    // Set up the logger(s)
    if (!config.cosm && !config.Pusher) {
        log.warn('No data logger found');
    }
    if (config.cosm) {
        log.info('Setting up xively');
        var CosmLogger = require('./cosm_logger');
        self.cosm = new CosmLogger(config.cosm);
    }
    if (config.pusher) {
        log.info('Setting up Pusher');
        var PusherLogger = require('./pusher_logger');
        self.realTimeFeed = new PusherLogger(config.pusher);
    }
...
OS_WStation.prototype.batteryLevel = function(level, device) {
	...
	if (level === 'low' && device) {
		...
		if (config.battery[device].latest_alert && (config.battery[device].latest_alert + 1000 * 3600 * 24) < Date.now()) {
			config.battery[device].latest_alert = Date.now();
			slack.send({
				channel: '#alerts',
				icon_emoji: ':battery:',
...
OS_WStation.prototype.decodeRainReport = function(data) {
    ...
    var obj = {};
    obj.reading = 'rain';
    obj.batteryLevel = self.batteryLevel(data[0], 'rain gauge');
...
OS_WStation.prototype.decodeTempAndHumidityReport = function(data) {
    ...
    obj.batteryLevel = self.batteryLevel(data[0], 'temp sensor ' + sensor);
    obj.sensor = sensor;
...
OS_WStation.prototype.decodeUVReport = function(data) {
    ...
    var obj = {};
    obj.reading = 'uv';
    obj.batteryLevel = self.batteryLevel(data[0], 'uv meter');
    obj.index = idx;
...
OS_WStation.prototype.decodeWindReport = function(data) {
    ...
    var obj = {};
    obj.reading = 'wind';
    obj.batteryLevel = self.batteryLevel(data[0], 'wind meter');
    obj.gust = gust;
    obj.average = avg;
    obj.direction = dir;
...
OS_WStation.prototype.decodeStatusReport = function(data) {
    ...
    var obj = {};
    obj.reading = 'status';
    obj.battery_level = self.batteryLevel(data[0], 'base station');
